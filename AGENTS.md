...
| **Business Systems Integration** (Track 3, discovery-priced) | $550 audit → $2,200 / $4,200 / $6,500 by tier | | |

Team: `project-manager` → `solution-architect` → `frontend-engineer` + `backend-engineer` (parallel where possible) → `qa-docs`.
...
2. **`solution-architect`** — define structure, stack, data flow, API needs, integrations, and technical tradeoffs. Category A only.
...
5. **`integrations-engineer`** — connect third-party systems, chatbots, AI agents, and no-code/low-code automation (Zapier/Make/n8n-style, webhooks, MCP). Category B.
